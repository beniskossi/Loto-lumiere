-- Création d'une fonction utilitaire pour récupérer les poids de prédiction avec mise en cache implicite / rapide
CREATE OR REPLACE FUNCTION public.get_prediction_weights()
RETURNS jsonb
LANGUAGE plpgsql
...
-- Mise à jour de calculate_composite_score pour utiliser la nouvelle fonction
...
BEGIN
  ...
  v_weights := public.get_prediction_weights();
  ...
  -- Score composite : applique les poids dynamiques sur les dimensions calculées
