...
export function generateOptimizedPrediction(
  ...
  // --- NOUVEAUX AXES ---
  // Double Gap Sequence Projector (Analyse et projection de la séquence des écarts des écarts)
  const doubleGapScores = calculateDoubleGapSequence(results);
  
  // Cadence Morphologique (Recherche rythmique et morphologie)
  const cadenceScores = calculateGapCadenceMorphology(results);
  ...
  for (let n = 1; n <= 90; n++) {
    ...
    // Bonus Double Gap (Poids très élevé, "avant tout autres poids")
    const doubleGapBonus = doubleGapScores.get(n) || 0;
    
    // Bonus Cadence (Poids très élevé)
    const cadenceBonus = cadenceScores.get(n) || 0;
    ...
    // "Les prédictions devraient prendre en compte ses deux algorithmes avant tout autres poids"
    const totalScore = freqScore * weights.frequency + 
                       ...
                       doubleGapBonus * 2.5 + // Poids extrêmement élevé (priorité absolue)
                       cadenceBonus * 2.0;    // Poids extrêmement élevé
