...
async function run() {
  console.log("Fetching multi-draw-prediction...");
  try {
    const res = await fetch(`${SUPABASE_URL}/functions/v1/multi-draw-prediction`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({ drawNames: ["Wari", "National"] })
    });
